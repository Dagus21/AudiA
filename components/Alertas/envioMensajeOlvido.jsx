...
const EnvioMensajeOlvidoExitoso = ({ setMostrarEnvioMensajeOlvidoExitoso }) => {
  
  useEffect(() => {
    const timer = setTimeout(() => {
        setMostrarEnvioMensajeOlvidoExitoso(false); // Ocultar la alerta después de 5 segundos
    }, 5000);

    return () => clearTimeout(timer); // Limpiar el temporizador cuando el componente se desmonte
  }, [setMostrarEnvioMensajeOlvidoExitoso ]);
  ...
  return (
    <div className="relative bg-green-50 border-l-4 border-green-500 text-green-800 p-6 rounded-lg shadow-lg w-1/3 mx-auto flex items-start space-x-4">
      ...
      <div className="flex-1">
        <h1 className="text-lg font-semibold">Se ha enviado un correo a tu direccion , revisa tu bandeja de entrada</h1>
      </div>
    </div>
  );
};

export default EnvioMensajeOlvidoExitoso;
