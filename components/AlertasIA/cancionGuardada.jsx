...
const CancionGuardadaExitoso = ({ setMostrarCancionGuardadaExitoso }) => {
  
  useEffect(() => {
    const timer = setTimeout(() => {
        setMostrarCancionGuardadaExitoso(false); // Ocultar la alerta después de 5 segundos
    }, 3000);

    return () => clearTimeout(timer); // Limpiar el temporizador cuando el componente se desmonte
  }, [setMostrarCancionGuardadaExitoso]);
  ...
  return (
    <div className="relative bg-green-50 border-l-4 border-green-500 text-green-800 p-6 rounded-lg shadow-lg w-1/3 mx-auto flex items-start space-x-4">
      ...
      <div className="flex-1">
        <h1 className="text-lg font-semibold">La canción se guardo correctamente.</h1>
      </div>
    </div>
  );
};

export default CancionGuardadaExitoso;